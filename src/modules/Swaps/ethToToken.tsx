...
export const EthToTokenSwaps: React.FC<Props> = ({ exchangeId, setSwapData, switchToEthSwap }) => {
  ...
  const handleSetEthNum = async (val: number) => {
    const tokenAmountSlippageResponse = await api.post('exchanges/token-amount/slippage/' + exchangeId, {
      amount: val
    });
    setEthNum(val);
    setTokenNum(Math.round(tokenAmountSlippageResponse.data.price * 1000) / 1000);
    setSlippage(Math.round(tokenAmountSlippageResponse.data.slippage * 1000) / 1000);
  }

  const handleSubmit = async (event: React.FormEvent) => {
    event.preventDefault();

    await api.post('exchanges/eth-to-token/' + exchangeId, {
      amount: ethNum,
  ...
  return (
    <div>
      ...
      <div className="mt-10">
        <h2 className='text-center text-3xl mb-5'>ETH <span className='font-bold text-green-600 cursor-pointer' onClick={() => switchToEthSwap(false)}> -&gt;</span> {exchangeData?.tokenSymbol}</h2>
        ...
        {/* @ts-ignore */}
        <h4 className='mb-5 text-xl'>Rate without slippage: {Math.round((Number(exchangeData?.tokenBalance / exchangeData?.ethBalance) * ethNum) * 1000) / 1000} {exchangeData?.tokenSymbol}</h4>
        ...
        <h3 className='text-red-400 text-2xl font-bold text-center mt-10'>Slippage: {slippage} {exchangeData?.tokenSymbol}</h3>
      </div>
    </div>
  );
}